...
void busca(vector<vector <int> > mat, vector<vector <Coordenada> > coordenadas, int i, int j)
{
	...
	if(i >= 0 && i < 5 && j >= 0 && j < 5 && !winner)
	{
		...
		else
		{
			...
			if(i + 1 < 5 && mat[i + 1][j] == 0 && !coordenadas[i][j].visitada)
			{
				busca(mat, coordenadas, i + 1, j);
			}
			if(i - 1 >= 0 && mat[i - 1][j] == 0 && !coordenadas[i][j].visitada)
			{
				busca(mat, coordenadas, i - 1, j);
			}
			if(j + 1 < 5 && mat[i][j + 1] == 0 && !coordenadas[i][j].visitada)
            {
                busca(mat, coordenadas, i, j + 1);
            }
			if(j - 1 >= 0 && mat[i][j - 1] == 0 && !coordenadas[i][j].visitada)
            {
                busca(mat, coordenadas, i, j - 1);
            }
		}
